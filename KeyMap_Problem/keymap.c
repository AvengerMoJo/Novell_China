...
static const char* default_keymap_path(const char* path)
{
	...
	if (!strchr(path, '/')) {
		snprintf(result, sizeof(result), "%s%s", "/lib/udev/keymaps/", path);
		return result;
	}
	return path;
}
...
static void print_key(struct input_event *event)
{
	...
	if (event->type == EV_KEY && event->value) {
		...
		if (keyname != NULL)
			printf("scan code: 0x%04X   key code: %s\n", 
                              (cur_scancode >= 0x80) ? (0xe000 + cur_scancode - 0x80) : cur_scancode ,
			    format_keyname(key_names[event->code]));
		else
			printf("scan code: 0xe0 0x%02X   key code: %03X\n", 
                              (cur_scancode >= 0x80) ? (cur_scancode - 0x80) : cur_scancode ,
			    event->code);
	}
}
...
static void interactive(int fd)
{
	...
	int run = 1;
	struct termios tio, tio_new;
	struct termios tio1, tio_new1;
        int fdflags;
        int fileout;
	...
	ioctl(fd, EVIOCGRAB, 1);
        ioctl(fileout,TCGETS, &tio);
        memcpy(&tio_new,&tio,sizeof(struct termios));
        tio_new.c_lflag &= ~(ICANON|ECHO); /* Clear ICANON and ECHO. */
        ioctl(fileout,TCSETS,&tio_new);
	...
	while (run) {
		...
		default:
			...
			//if (ev.type == EV_KEY && ev.code < KEY_WIMAX && ev.value == 1)
			//	run = 0;
